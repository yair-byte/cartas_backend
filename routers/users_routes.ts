...
import { obtenerTablaCompleta , guardarNuevoDato, obtenerRegistroCompleto, obtenerRegistroPorID, verificarPermisos } from '../services/services';
...
routerUsuario.post('/nuevousuario', async (req: Request, res: Response) => {
  try {
    ...
    const registroInsertado: Usuario[] = await guardarNuevoDato<Usuario>(nuevoUsuario, NameTables.Usuario);
...
routerUsuario.post('/login', async (req: Request, res: Response) => {
  try {
    ...
    const arrUsuario = await obtenerRegistroCompleto<Usuario>(usuarioAux.nombre_usuario, 'nombre_usuario', NameTables.Usuario);
...
//ejemplo 
routerUsuario.delete('/', verificarPermisos(Permission.UsuarioComun) , (_req: Request, res: Response) => {
  try {
